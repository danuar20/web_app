...
ALL_KPI_DEFS = [
    ("payloadChart",       "Payload",              "GB",   0,    None,
     "SUM(total_payload)::numeric/1024",  False),
    ("tchTrafficChart",    "TCH Traffic",           "Erl",  0,    None,
     "ROUND(SUM(tch_traffic)::numeric, 2)",         False),
    ("sdcchTrafficChart",  "SDCCH Traffic",         "Erl",  0,    None,
     "ROUND(SUM(sdcch_traffic)::numeric, 2)",       False),
    ("fullRateChart",      "Full Rate Traffic",     "Erl",  0,    None,
     'ROUND(SUM("Offic_full_traffic")::numeric, 2)', False),
    ("halfRateChart",      "Half Rate Traffic",     "Erl",  0,    None,
    ...
    ("interfChart",        "Interference",          "%",    None,    100,
     ...
     True),
    ("dlMosChart",         "DL MOS",                "",     None,    None,
     "ROUND(AVG(mos_dl)::numeric, 2)",              False),
    ("ulMosChart",         "UL MOS",                "",     None,    None,
     "ROUND(AVG(mos_ul)::numeric, 2)",              False),
    ("sdToTchChart",       "SD to TCH",             "%",    None,    100,
     "ROUND((AVG(sd_to_tch)*100)::numeric, 2)",     False),
    ("cstChart",           "CST",                   "ms",   0,    None,
    ...
    ("ulQualChart",        "UL Qual",               "%",    None,    None,
     ...
     True),
    ("gprsDlThpChart",     "GPRS DL Thp",           "Kbps", 0,    None,
     "ROUND(AVG(gprs_dl_thp)::numeric, 2)",         False),
    ("edgeDlThpChart",     "EDGE DL Thp",           "Kbps", 0,    None,
     "ROUND(AVG(edge_dl_thp)::numeric, 2)",         False),
    ("gprsPayloadChart",   "GPRS Payload",          "GB",   0,    None,
     "SUM(gprs_payload)::numeric/1024",             False),
    ("edgePayloadChart",   "EDGE Payload",          "GB",   0,    None,
