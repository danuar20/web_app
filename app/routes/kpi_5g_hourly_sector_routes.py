...
def kpi_5g_hourly_sector():
    ...
    ALL_KPI_DEFS = [
        ...
        ("cellDlThpChart", "Cell DL Thp",            "Mbps", None, 0, None,
         'ROUND((AVG(cell_throughput_dl_xhj) )::numeric, 2)', False),
        ("cellUlThpChart", "Cell UL Thp",            "Mbps", None, 0, None,
         'ROUND((AVG(cell_throughput_ul_xhj))::numeric, 2)', False),
        ("dlThpChart",     "User DL Thp",            "Mbps", None, None, None,
         'CASE WHEN SUM(denum_user_throughput_dl_xhj) > 0 THEN ROUND((SUM(num_user_throughput_dl_xhj) / SUM(denum_user_throughput_dl_xhj) )::numeric, 2) ELSE NULL END', False),
        ("ulThpChart",     "User UL Thp",            "Mbps", None, None, None,
         'CASE WHEN SUM(denum_user_throughput_ul_xhj) > 0 THEN ROUND((SUM(num_user_throughput_ul_xhj) / SUM(denum_user_throughput_ul_xhj) )::numeric, 2) ELSE NULL END', False),
