...
ALL_KPI_DEFS = [
    ...
    ("availChart", "Availability", "%", None, 100, 'CASE WHEN SUM(denum_availability_xhj) > 0 THEN ROUND((SUM(num_availability_xhj) / SUM(denum_availability_xhj) * 100.0)::numeric, 2) ELSE NULL END', False),
    ...
    ("accessibilityChart", "NR Accessibility", "%", None, 100, 'CASE WHEN SUM(number_of_sn_add_requests) > 0 THEN ROUND((SUM(num_sn_setup_success_rate_xhj) / SUM(number_of_sn_add_requests) * 100.0)::numeric, 2) ELSE NULL END', False),
    ("retainabilityChart", "NR Retainability", "%", None, 100, 'CASE WHEN SUM(denum_nr_retainability_xhj) > 0 THEN ROUND((100.0 - (SUM(num_nr_retainability_xhj) / SUM(denum_nr_retainability_xhj) * 100.0))::numeric, 2) ELSE NULL END', False),
    ("mobilityChart", "NR Mobility SR", "%", None, 100, 'CASE WHEN SUM(nr_mobility_success_rate_denum) > 0 THEN ROUND((SUM(nr_mobility_success_rate_num) / SUM(nr_mobility_success_rate_denum) * 100.0)::numeric, 2) ELSE NULL END', False),
    ("dlPrbChart", "DL PRB", "%", 0, 100, 'CASE WHEN SUM(denum_prb_utilization_dl_xhj) > 0 THEN ROUND((SUM(num_prb_utilization_dl_xhj) / SUM(denum_prb_utilization_dl_xhj) * 100.0)::numeric, 2) ELSE NULL END', False),
    ("ulPrbChart", "UL PRB", "%", 0, 100, 'CASE WHEN SUM(denum_prb_utilization_ul_xhj) > 0 THEN ROUND((SUM(num_prb_utilization_ul_xhj) / SUM(denum_prb_utilization_ul_xhj) * 100.0)::numeric, 2) ELSE NULL END', False),
    ("cellDlThpChart", "Cell DL Thp", "Mbps", 0, None, 'ROUND((AVG(cell_throughput_dl_xhj))::numeric, 2)', False),
    ("cellUlThpChart", "Cell UL Thp", "Mbps", 0, None, 'ROUND((AVG(cell_throughput_ul_xhj) )::numeric, 2)', False),
    ...
    ("seChart", "SE", "", 0, None, 'CASE WHEN SUM(spectrum_eff_bps_lw_denum) > 0 THEN ROUND((SUM(spectrum_eff_bps_lw_num) / SUM(spectrum_eff_bps_lw_denum))::numeric, 4) ELSE NULL END', False),
    ("cqiChart", "CQI", "", 0, None, 'CASE WHEN SUM(denum_average_cqi_xhj) > 0 THEN ROUND((SUM(num_average_cqi_xhj) / SUM(denum_average_cqi_xhj))::numeric, 2) ELSE NULL END', False),
    ("ulIntChart", "UL Interference", "dBm", -120, -90, 'ROUND(AVG(avg_uplink_interference_xhj)::numeric, 2)', True),
    ("plChart", "Packet Loss", "%", 0, None, 'CASE WHEN SUM(denum_packet_loss_xhj) > 0 THEN ROUND((SUM(num_packet_loss_xhj) / SUM(denum_packet_loss_xhj) * 100.0)::numeric, 2) ELSE NULL END', True),
    ("latDlChart", "Latency DL", "ms", 0, None, 'CASE WHEN SUM(denum_latency_dl_xhj) > 0 THEN ROUND((SUM(num_latency_dl_xhj) / SUM(denum_latency_dl_xhj))::numeric, 2) ELSE NULL END', True),
    ("latUlChart", "Latency UL", "ms", 0, None, 'CASE WHEN SUM(denum_latency_ul_xhj) > 0 THEN ROUND((SUM(num_latency_ul_xhj) / SUM(denum_latency_ul_xhj))::numeric, 2) ELSE NULL END', True),
]
