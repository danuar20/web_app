...
def create_table_if_not_exists(conn):
    ...
    CREATE TABLE IF NOT EXISTS "4g_kpi_zte_daily" (
        ...
        siteid VARCHAR(255),
        total_payload_mb DOUBLE PRECISION,
...
def aggregate_date(conn, target_date):
    cur = conn.cursor()
    print(f"Aggregating 4G data for {target_date}...")

    cur.execute('DELETE FROM "4g_kpi_zte_daily" WHERE kpi_date = %s', [target_date])

    sql = '''
    INSERT INTO "4g_kpi_zte_daily" (
        kpi_date, nsa, city, subnetwork_name, siteid,
        total_payload_mb, dl_traffic_volume, ul_traffic_volume,
        ...
        avg_rsrp_dbm, avg_rsrq_dbm, avg_cell_rssi,
    ...
    FROM "4g_kpi_zte"
    WHERE DATE(datehour) = %s
      AND nsa IS NOT NULL
      AND city IS NOT NULL
      AND subnetwork_name IS NOT NULL
      AND siteid IS NOT NULL
    GROUP BY DATE(datehour), nsa, city, subnetwork_name, siteid;
    '''
    ...
    conn.commit()
    cur.close()
    print(f"-> Inserted {inserted} daily site-level records for {target_date}.")
...
def main():
    ...
    if args.start_date:
        cur = conn.cursor()
        cur.execute('SELECT MAX(DATE(datehour)) FROM "4g_kpi_zte"')
        ...
        while current <= max_date:
            aggregate_date(conn, current.strftime("%Y-%m-%d"))
            current += timedelta(days=1)

    elif args.date:
        aggregate_date(conn, args.date)
    else:
        cur = conn.cursor()
        cur.execute('SELECT MAX(DATE(datehour)) FROM "4g_kpi_zte"')
        ...
        if max_daily_date and max_daily_date < max_raw_date:
            ...
            current = max_daily_date + timedelta(days=1)
            while current <= max_raw_date:
                aggregate_date(conn, current.strftime("%Y-%m-%d"))
                current += timedelta(days=1)
        else:
            print(f"Found latest data date: {max_raw_date}")
            for i in range(args.days):
                target = max_raw_date - timedelta(days=i)
                aggregate_date(conn, target.strftime("%Y-%m-%d"))
